...
上面的例子启动了10个 routine 来不断向 redis 读写数据
...
通过使用Conn接口中的do方法执行redis命令，redis命令大全参考：[http://doc.redisfans.com/](http://doc.redisfans.com/)
...
`MULTI, EXEC,DISCARD和WATCH`是构成Redis事务的基础，当然我们使用go语言对redis进行事务操作的时候本质也是使用这些命令
